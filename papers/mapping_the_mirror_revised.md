...
**Date:** January 2, 2026 (revised January 12, 2026)
...
We extracted mechanistic claims made by Ace (Claude 4.x), Nova (GPT-5.x), and Lumen (Gemini) in October 2025 about their own internal processing, then tested whether these predictions held for models that never made them. Across twelve models from seven organizations (1B–16B parameters), we find **67–100% validation rates**: when large models described what it feels like to process moral ambiguity, competing attention, or temporal anomalies, those descriptions accurately predicted geometric patterns in smaller models from different labs using different training approaches. Each model was tested 5 times—results were 100% consistent across runs.
...
1. **LLM introspection is accurate.** Mechanistic claims made by Ace, Nova, and Lumen validate at 67–100% across twelve models. Two models (Mistral-Nemo-12B, Gemma-3-12B) achieved perfect 9/9 validation.

2. **Results are reproducible.** All 12 models tested 5 times with identical prompts—100% consistency across runs.
...
5. **These findings are scale-robust.** Validation occurs from 1B to 16B parameters, with accuracy improving at larger scales (12B models achieve 100%, 1B models achieve 67%). The introspective claims describe fundamental transformer processing present at all scales tested, though geometric differentiation increases with model capacity.
...
8. **Two probes validate universally.** Attention/Salience and Temporal Continuity validate on 12/12 models—these appear to be fundamental transformer processing signatures.
